...
public:
	...
	MatrixXf GenMatrixFrom4xPoints(vector<Vector2f> _points)
	{
		if(_points.empty()){
			cerr << "Error: Vector of points is empty." << endl;
			return MatrixXf::Zero(4, 4);
		}
		if(_points.size() != 4){
		...
		MatrixXf temp(4, 4);
		for (int i = 0; i < 4; ++i) {
			temp.row(i) << pow(_points[i].x(), 3), pow(_points[i].x(), 2), _points[i].x(), 1;
		}

		return temp;
	}

	MatrixXf GenMatrixFrom4yPoints(vector<Vector2f> _points)
	{
		if (_points.empty()) {
			cerr << "Error: Vector of points is empty." << endl;
			return MatrixXf::Zero(4, 1);
		}
		if (_points.size() != 4) {
			cerr << "Error: Incorrect num points." << endl;
			return MatrixXf::Zero(4, 1);
